...
class TorneosController
{
    ...
    public function detalle($id)
    {
        ...
        /*var_dump([
            ...
            'rol' => AuthMiddleware::tieneRol('jugador'),
    ...
    public function guardarRonda($id)
    {
        ...
        $this->validarFechasRonda((int)$id, $datos['fechainicio'], $datos['fechafin']);
    ...
    public function guardarPartido($id)
    {
        ...
        $this->validarFechaPartido((int)$id, $datos['idronda'], $datos['fecha']);
    ...
    private function fechaValida($valor, $formato)
    {
        $zonaHoraria = new DateTimeZone('America/La_Paz');
        $fecha = DateTimeImmutable::createFromFormat('!' . $formato, $valor, $zonaHoraria);
        $errores = DateTimeImmutable::getLastErrors();

        if (!$fecha || ($errores !== false && ($errores['warning_count'] > 0 || $errores['error_count'] > 0)) || $fecha->format($formato) !== $valor) {
            return null;
        }
    ...
    private function validarFechasRonda($idtorneo, $fechainicio, $fechafin)
    {
        $inicio = $this->fechaValida($fechainicio, 'Y-m-d\\TH:i');
        $fin = $this->fechaValida($fechafin, 'Y-m-d\\TH:i');
        $torneo = $this->torneos->obtenerTorneoPorId($idtorneo);

        if (!$inicio || !$fin || !$torneo || (int)$torneo['idorganizador'] !== (int)$_SESSION['usuario_id']) {
            http_response_code(400);
            $this->responder(false, 'Las fechas de la ronda no son válidas.');
        }

        $zonaHoraria = new DateTimeZone('America/La_Paz');
        $inicioTorneo = new DateTimeImmutable($torneo['fechainicio'] . ' 00:00:00', $zonaHoraria);
        $finTorneo = new DateTimeImmutable($torneo['fechafin'] . ' 23:59:59', $zonaHoraria);

        if ($inicio >= $fin) {
            http_response_code(400);
            $this->responder(false, 'La fecha de inicio de la ronda debe ser anterior a su fecha de finalización.');
        }

        if ($inicio < $inicioTorneo || $fin > $finTorneo) {
            http_response_code(400);
            $this->responder(false, 'Las fechas de la ronda deben estar dentro del período del torneo.');
        }
    }

    private function validarFechaPartido($idtorneo, $idronda, $fecha)
    {
        $fechaPartido = $this->fechaValida($fecha, 'Y-m-d\\TH:i');
        $ronda = $this->torneos->obtenerRondaGestionable($idronda, $idtorneo, $_SESSION['usuario_id']);

        if (!$fechaPartido || !$ronda || !$ronda['fechainicio'] || !$ronda['fechafin']) {
            http_response_code(400);
            $this->responder(false, 'La fecha del partido o la ronda no son válidas.');
        }

        $zonaHoraria = new DateTimeZone('America/La_Paz');
        $inicioRonda = new DateTimeImmutable($ronda['fechainicio'], $zonaHoraria);
        $finRonda = new DateTimeImmutable($ronda['fechafin'], $zonaHoraria);

        if ($fechaPartido < $inicioRonda || $fechaPartido > $finRonda) {
            http_response_code(400);
            $this->responder(false, 'La fecha del partido debe estar dentro del período de su ronda.');
        }
    }
    ...
    public function guardar()
    {
        ...
        $inicio = $this->fechaValida($fechainicio, 'Y-m-d');
        $fin = $this->fechaValida($fechafin, 'Y-m-d');
        $hoy = new DateTimeImmutable('today', new DateTimeZone('America/La_Paz'));

        if (!$inicio || !$fin || $inicio < $hoy) {
            http_response_code(400);
            $this->responder(false, 'La fecha de inicio del torneo debe ser igual o posterior a la fecha actual.');
        }
